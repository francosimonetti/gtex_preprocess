...
def parse_args():
    ...
    parser.add_argument('--rpkm',
                        ...
                        dest='rpkmpath',
                        metavar='FILE',
                        help='input expression matrix file for a given tissue')

    parser.add_argument('--counts',
                        type=str,
                        dest='countspath',
                        metavar='FILE',
                        help='input counts matrix file for all tissues')

    parser.add_argument('--tissue',
                        type=str,
                        dest='tissue',
                        metavar='STR',
                        help='exact acronym of the tissue')
...
def read_matrix(gct_file, donor_ids):
...
# tissue_rpkm = opts.rpkmpath + f"{opts.tissue}.txt"
# tissue_cnts = opts.countspath + f"{opts.tissue}.txt"

# if ( not os.path.exists(tissue_rpkm) ) or ( not os.path.exists(tissue_cnts) ):
#     print(f"ERROR file does not exists: {tissue_rpkm} or {tissue_cnts}")
#     raise

donor_ids = get_donors(opts.donorspath)
# expression_df = read_matrix(tissue_rpkm, donor_ids)
# counts_df = read_matrix(tissue_cnts, donor_ids)
expression_df = read_matrix(opts.rpkmpath, donor_ids)
counts_df = read_matrix(opts.countspath, donor_ids)
...
if not os.path.exists(os.path.join(opts.outdir,"tpms_qcfilter")):
    os.makedirs(os.path.join(opts.outdir,"tpms_qcfilter"))
qc_tpm_expr.to_csv(os.path.join(opts.outdir,"tpms_qcfilter","{:s}.tpms_qcfilter.txt.gz".format(opts.tissue)), sep="\t")
qc_counts.to_csv(os.path.join(opts.outdir,"counts_qcfilter","{:s}.counts_qcfilter.txt.gz".format(opts.tissue)), sep="\t")
...
qn_expr.to_csv(os.path.join(opts.outdir,"qn","{:s}.qn.txt.gz".format(opts.tissue)), sep="\t")
tmm_expr.to_csv(os.path.join(opts.outdir,"tmm","{:s}.tmm.txt.gz".format(opts.tissue)), sep="\t")
